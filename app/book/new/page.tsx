import { getBookTemplateById } from "@/actions/templates";
...
export default async function NewBookPage(props: Props) {
  const { templateId } = await props.searchParams;

  let templateData = "";
  if (templateId) {
    const template = await getBookTemplateById(templateId);
    if (template) {
